...
import { APIUserAbortError } from '@terminaldotshop/sdk';
import { Headers } from '@terminaldotshop/sdk/core';
...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['TERMINAL_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Terminal({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
    ...
    test('in request options overridden by env variable', () => {
      process.env['TERMINAL_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Terminal({ bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Terminal({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Terminal({ bearerToken: 'My Bearer Token' });
    expect(client2.maxRetries).toEqual(2);
  });

  test('with environment variable arguments', () => {
    // set options via env var
    process.env['TERMINAL_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new Terminal();
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Terminal({ bearerToken: 'My Bearer Token', fetch: testFetch });
